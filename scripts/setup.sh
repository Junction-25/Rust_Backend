...
#     sudo -u postgres psql -d real_estate_db -c "SELECT 1 FROM pg_roles WHERE rolname='$current_user'" | grep -q 1 || \
#     sudo -u postgres createuser -d real_estate_db --interactive --pwprompt $current_user
    
#     # Create database
#     sudo -u postgres createdb -d real_estate_db 2>/dev/null || echo "Database may already exist"
