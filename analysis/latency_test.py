...
class RecommendationLatencyTester:
    ...
    def test_bulk_recommendations(self, contact_ids: List[int], limit_per_contact: Optional[int] = None,
                                min_score: Optional[float] = None) -> LatencyResult:
    ...
    def save_results_to_csv(self, filename: str = "analysis/latency_results.csv") -> None:
        ...
        with open(filename, 'w', newline='') as csvfile:
            fieldnames = [
                'timestamp', 'endpoint', 'contact_id', 'response_time_ms',
    ...
    def print_summary_stats(self) -> None:
        ...
        if successful_results:
            ...
            single_contact_results = [r for r in successful_results if r.endpoint == "single_contact"]
            bulk_results = [r for r in successful_results if r.endpoint == "bulk_recommendations"]
            
            if single_contact_results:
                single_times = [r.response_time_ms for r in single_contact_results]
                print(f"\nSingle Contact Endpoint:")
                print(f"  Tests: {len(single_contact_results)}")
...
def main():
    ...
    tester.run_test_suite(args.iterations, args.contact_ids)
